...
def admin_dashboard():
    db = get_db()
    try:
        filter_admin_id = request.form.get('filter_admin_id') if request.method == 'POST' else None

        # Fetch all complaints with student and admin details, optionally filtered by admin
        query = '''
            SELECT c.*, 
        ...
        return render_template('admin/dashboard.html', 
                               ...
                               selected_admin_id=filter_admin_id)
    except sqlite3.Error as e:
        flash(f'Error retrieving complaints: {str(e)}', 'danger')
        return redirect(url_for('home'))
    finally:
